...
async function setupDirectories(remotePath, username, host, password) {
    const spinner = ora("Setting up directories...").start();
    try {
        spinner.text = "Deleting existing directories...";
        await execPromise(`sshpass -p "${password}" ssh ${username}@${host} "rm -rf ${remotePath}${api_path} ${remotePath}${admin_panel_path} && mkdir -p ${remotePath}${api_path} ${remotePath}${admin_panel_path}" `);
    ...
    try {
        ...
        for (const [index, role] of userRoles.entries()) {
            ...
            try {
                ...
                spinner.fail(`Failed to insert role "${role.name}": ${error.response?.data?.message || error.message}`);
            }
        }
        ...
        throw new Error("Error seeding user roles: " + error.message);
    }
}
...
async function copyToServer(remotePath, username, host, password) {
    const spinner = ora("Copying files to server...").start();
    try {
        spinner.text = "Copying admin-panel dist folder...";
        await execPromise(`sshpass -p "${password}" scp -r ${path.resolve(__dirname, "../admin-panel/dist")} ${username}@${host}:${remotePath}${admin_panel_path}`);
        spinner.text = "Copying api dist folder...";
        await execPromise(`sshpass -p "${password}" scp -r ${path.resolve(__dirname, "../api/dist")} ${username}@${host}:${remotePath}${api_path}`);
...
async function setupStartupScripts(remotePath, username, host, password) {
...
`;
        ...
        await execPromise(`sshpass -p "${password}" scp ${tempScriptPath} ${username}@${host}:/tmp/startup.sh`);
        await execPromise(`sshpass -p "${password}" ssh ${username}@${host} "sudo mv /tmp/startup.sh /etc/init.d/story-world && sudo chmod +x /etc/init.d/story-world && sudo update-rc.d story-world defaults"`);
